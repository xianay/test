...
public class ClientJFrame extends javax.swing.JFrame {
	...
	private String IP = "127.0.0.1";  //服务端 ip
	
	private JButton b_connect = new JButton("连接");
	private JLabel l_connect = new JLabel("对方IP:");
	private JTextField tf_connect = new JTextField(IP,10);
	private JPanel p_connect = new JPanel();
	
	private void _runThread()
	{
		this.add(p_connect, BorderLayout.NORTH);
		p_connect.setLayout(new FlowLayout());
		p_connect.add(l_connect);
		p_connect.add(tf_connect);
		p_connect.add(b_connect);
		b_connect.addActionListener(new ActionListener() {
		
			public void actionPerformed(ActionEvent e) {
				if(e.getActionCommand().equals("连接"))
				{
					IP = tf_connect.getText();
					b_connect.setEnabled(false);
					tf_connect.setEnabled(false);
					chatT.start();
					playT.start();
				}
			}
	...
	private void receive()
	{
		while (true) {
			...
			try {
				if (dis.available() > 0) {
					int row = dis.readInt(); //接收服务端 当前棋子的 x[棋盘坐标]
					int col = dis.readInt(); //接收服务端 当前棋子的 y[棋盘坐标]
					canPlace(row, col, Chess.BLACK);  //放置一颗黑色棋子
				}
				Thread.sleep(100);
			} catch (IOException e) {
			} catch (InterruptedException e) {
	...
	void myInit() {
		...
		jPanel_Gobang.addMouseListener(new MouseAdapter() {
			public void mouseClicked(MouseEvent e) {
				if(isPort1Ready && isPort2Ready)
				if (addChess(e.getPoint(), Chess.WHITE)) {
					if(pos != null)
					{
						send();
						pos = null;
